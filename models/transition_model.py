...
class TransitionModel:
    def __init__(self,
        ...
        self.model = EnsembleModel(
            obs_dim=obs_dim, action_dim=action_dim, device=util.device, pretrain=pretrain, **kwargs['model'])
    ...
    def update(self, data_batch, deterministic=False):
        ...
        train_transition_loss = train_mse_loss + train_var_loss + self._phi * train_val_loss
    ...
    def predict(self, obs, act, deterministic=False):
        ...
        terminals = self.static_fns(obs.cpu().numpy(), act.cpu().numpy(), next_obs.cpu().numpy())
        terminals = torch.as_tensor(terminals, dtype=torch.bool).squeeze().to(self.device)
    ...
    def load_model(self, model_save_dir, name):
        ...
        self.model.load_state_dicts(model_params, self.pretrain)
